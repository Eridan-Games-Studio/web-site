...
const callingData = {
    investigator: {
        ...
        abilities: [
            ...
            "Street-hardened combat skills &mdash; brawler or sharpshooter",
    ...
    rumormonger: {
        ...
        description: `
            ...
            <p>You maintain a <strong>secret identity</strong> &mdash; perhaps more than one &mdash; complete with papers, accounts, and alibis. Swapping between personas takes planning, but you can shift in moments when needed. Your <strong>unspoken messages</strong> reach anyone in Haven through strange coincidences, and you're always prepared, always one step ahead.</p>

            <p>You live comfortably because secrets sell. Sharp clothes, a decent apartment, and enough money to keep up appearances. Information networks whisper your name with respect and fear, knowing you can make or break reputations with a well-placed word. In Haven, <strong>truth is the most dangerous currency</strong> &mdash; and you print the bills.</p>
        `,
        abilities: [
    ...
    demonologist: {
        ...
        abilities: [
            ...
            "Power at a price &mdash; every gift has consequences"
        ]
    },
    ...
    haunted: {
        ...
        description: `
            <p>You are the <strong>Haunted One</strong>, someone who's never truly alone. A spirit walks beside you &mdash; unseen by most, half-remembered by all. It hums in dead radios, leaves fog on mirrors, and sometimes borrows your voice when you're not paying attention. Maybe it's unfinished business. Maybe it's a promise neither of you can keep. Maybe it's just chance &mdash; a collision between the living and whatever waits beyond.</p>

            <p>Your <strong>Ghost in the Wires</strong> ability is terrifying and useful in equal measure. By commanding your spectral companion, you can possess objects or even corpses &mdash; a car drives itself, a camera rewinds its footage, or the dead shuffle to your side waiting for orders. The spirit acts through your intent, but always with its own eerie touch: lights dim, metal warps, static fills the air.</p>

            <p>You can <strong>Summon the Departed</strong>, calling to ghosts of the recently dead through technology &mdash; they answer over radios, phones, crackling speakers. Their cooperation isn't guaranteed, but they always respond. Your <strong>Obscuring Aura</strong> makes people's eyes slide off you, cameras glitch, and timid spirits grow strangely friendly. You exist half-real, caught between worlds.</p>

            <p>Work is hard to keep when lights flicker wherever you go. People avoid you, jobs don't last, and you live where no one asks questions. Your small apartment is your refuge, along with a trinket you use to communicate &mdash; a radio, tape recorder, or handheld mirror. The spirit that haunts you might be a burden, but it's also your closest companion. In Haven, a city that's already haunted, <strong>you fit right in &mdash; for better or worse</strong>.</p>
        `,
        abilities: [
            ...
            "Technology haunting &mdash; lights, radios, screens react to you"
        ]
    }
};
...
const validationList = document.getElementById('validation-summary-list');
...
function showStep(stepNum, { focus = true } = {}) {
    ...
    currentStep = stepNum;
    if (progressStep) progressStep.textContent = String(stepNum);
    updateProgress();

    // Scroll to top of progress bar so user sees they advanced
    if (focus) {
        const progressEl = document.getElementById('quiz-progress');
        if (progressEl) progressEl.scrollIntoView({ behavior: 'smooth', block: 'start' });
        const heading = steps[stepNum - 1].querySelector('.step-heading');
        if (heading) {
            heading.setAttribute('tabindex', '-1');
            // Defer focus until after smooth scroll begins so screen readers announce
...
function countAnswered() {
    ...
    for (let i = 1; i <= TOTAL_QUESTIONS; i++) {
        if (form.elements[`q${i}`] && form.elements[`q${i}`].value) n++;
    }
    return n;
}
...
function updateProgress() {
    ...
    if (progressTrack) progressTrack.setAttribute('aria-valuenow', String(answered));
}

function getMissingInStep(stepNum) {
    const section = steps[stepNum - 1];
    const groups = {};
    ...
    return Object.entries(groups)
        ...
        .map(([name]) => Number(name.replace('q', '')));
}
...
function flagQuestions(numbers) {
    document.querySelectorAll('.question.is-missing').forEach(q => q.classList.remove('is-missing'));
    numbers.forEach(n => {
        const q = document.getElementById(`question-${n}`);
        if (q) q.classList.add('is-missing');
    });
}

function jumpToQuestion(num) {
    // Find the step that contains this question
    for (const section of steps) {
        if (section.querySelector(`#question-${num}`)) {
            const targetStep = Number(section.dataset.step);
            if (targetStep !== currentStep) showStep(targetStep, { focus: false });
            break;
        }
    }
    setTimeout(() => {
...
function showValidationSummary(missing, { scopeToStep = false } = {}) {
    ...
    const text = scopeToStep
        ? `Please answer the following before continuing: `
        : `Please answer the following questions before proceeding: `;
    document.getElementById('validation-summary-text').textContent = text;

    validationSummary.hidden = false;
    flagQuestions(missing);
}
...
form.addEventListener('click', e => {
    ...
    if (action === 'next') {
        const missingHere = getMissingInStep(currentStep);
        if (missingHere.length > 0) {
            // Inline validation: highlight missing questions in this step
            flagQuestions(missingHere);
            const firstMissing = document.getElementById(`question-${missingHere[0]}`);
            if (firstMissing) {
                firstMissing.scrollIntoView({ behavior: 'smooth', block: 'center' });
                const radio = firstMissing.querySelector('input[type="radio"]');
...
// Update progress + clear flag as user answers
...
form.addEventListener('submit', function(e) {
    ...
    if (missing.length > 0) {
        showValidationSummary(missing, { scopeToStep: false });
        // Scroll the validation summary into view
    ...
    // Tally
    const scores = { investigator: 0, rumormonger: 0, demonologist: 0, agent: 0, haunted: 0 };
...
function displayResult(calling, scores) {
    ...
    for (const [callingName, score] of sortedScores) {
        const pct = (score / TOTAL_QUESTIONS * 100).toFixed(0);
        const display = callingData[callingName].name;
        html += `
            <div class="score-item">
                ...
                <div class="score-bar-container">
                    <div class="score-bar" style="width: 0%;" data-target="${pct}"></div>
                </div>
            </div>
        `;
    }
    ...
    requestAnimationFrame(() => {
        resultContent.querySelectorAll('.score-bar').forEach(bar => {
