...
function displayResult(calling, scores) {
    ...
    // Record the vote for this calling
    recordVote(calling);
...
const POLL_STORAGE_KEY = 'havenCallingPollResults';
    ...
    if (stored) {
        try {
            ...
            if (typeof parsed === 'object' && parsed !== null) {
                const validCallings = ['investigator', 'rumormonger', 'demonologist', 'agent', 'haunted'];
                const validated = {};

                // Only accept valid callings with valid counts
                validCallings.forEach(calling => {
                    const count = parsed[calling];
                    validated[calling] = (typeof count === 'number' && count >= 0 && Number.isInteger(count)) ? count : 0;
                });
...
// Record a vote for a calling
function recordVote(calling) {
    // Validate calling name
    const validCallings = ['investigator', 'rumormonger', 'demonologist', 'agent', 'haunted'];
    if (!validCallings.includes(calling)) {
        console.error('Invalid calling name:', calling);
        return;
    }

    const results = getPollResults();
    if (results.hasOwnProperty(calling)) {
        results[calling]++;
...
// Validate and sanitize calling name
function isValidCalling(calling) {
    const validCallings = ['investigator', 'rumormonger', 'demonologist', 'agent', 'haunted'];
    return validCallings.includes(calling);
}
...
function displayPollResults() {
    ...
    validCallings.forEach(calling => {
        const count = results[calling];
        // Ensure count is a valid non-negative integer
        sanitizedResults[calling] = (typeof count === 'number' && count >= 0 && Number.isInteger(count)) ? count : 0;
    });

    // Calculate total votes
    const total = Object.values(sanitizedResults).reduce((sum, count) => sum + count, 0);

    // Calling display names
    const callingNames = {
    ...
    const sortedResults = Object.entries(sanitizedResults)
        .sort((a, b) => b[1] - a[1]);
    ...
    if (total === 0) {
        ...
        cell.textContent = 'No votes recorded yet. Be the first to take the quiz!';
        row.appendChild(cell);
        tbody.appendChild(row);
    } else {
        sortedResults.forEach(([calling, count]) => {
            // Only process valid callings
            if (!isValidCalling(calling)) return;

            const percentage = total > 0 ? ((count / total) * 100).toFixed(1) : 0;

            // Create row using DOM manipulation (safer than innerHTML)
            const row = document.createElement('tr');
            row.className = `${calling}-row`;

            // Calling name cell
            const nameCell = document.createElement('td');
            nameCell.className = 'calling-name';
            nameCell.textContent = callingNames[calling];
            ...
            if (percentage > 10) {
                ...
                barPercentage.className = 'visual-bar-percentage';
    ...
    setTimeout(() => {
        document.querySelectorAll('.visual-bar-fill').forEach(bar => {
